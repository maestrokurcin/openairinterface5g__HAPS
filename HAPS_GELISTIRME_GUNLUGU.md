...
### Adım 46 — Çoklu-ışın (Multi-beam) test konfigürasyonu

**Tarih**: 2026-09-06 · Test Günlüğü Deney 24 ile birlikte.

`[Dosya] haps_test/gnb.haps_mobile_ntn_38811_beam.conf` (YENİ)
`+ Eklendi:` `gnb.haps_mobile_ntn_38811.conf`'un kopyası; `rfsimulator` bloğuna
`enable_beams = 1; beam_map = 1; beam_gains = "0,-4,-9,-16";`.

`[Dosya] haps_test/nrue.haps_mobile_ntn_38811_beam.conf` (YENİ)
`+ Eklendi:` `nrue.haps_mobile_ntn_38811.conf`'un kopyası; `rfsimulator` bloğuna
`enable_beams = 1; beam_map = 1; beam_gains = "0,-4,-9,-16";` (UE'nin ışını
koşu başına `beam_map` ile seçilir: 1=ışın0, 2=ışın1, 4=ışın2, 8=ışın3).

`# Gerekçe:` rfsimulator'ın "basitleştirilmiş ışın simülasyonu" özelliği
(`radio/rfsimulator/simulator.cpp`, `RFSIMULATOR_PARAMS_DESC` içindeki
`enable_beams`/`beam_map`/`beam_gains`). `process_gains()` verilen listeden
Toeplitz bir matris kuruyor: `M[i][j] = liste[|i−j|]` (dB, negatif = kayıp),
yani köşegen 0 (ışınlar hizalı) ve ceza `|gNB_ışın − UE_ışın|` ile büyüyor.
`combine_received_beams()` (satır ~1168) alıcı tarafında
`M[rx_beam][tx_beam]`'i lineer kazanç olarak örneklere çarpıyor —
**HAPS kanal modelinden bağımsız, onun üstüne ek bir yol kaybı terimi**.
`beam_gains` **hem sunucuda hem istemcide** verilmeli (alıcıda uygulandığı için,
UE de DL alıcısıdır) ve iki listede aynı olmalı — eksikse
`get_rx_gain_db()` `AssertFatal` ile çöküyor (bu deneyde önce yaşandı).

**Kaynak kodda değişiklik yok** — yalnızca yeni test konfigürasyon dosyaları.
Not: telnet ile çalışma-zamanı ışın değiştirme (`rfsimu setbeam`) bu derlemede
`libtelnetsrv.so` derlenmediği için kullanılamadı; ışın ofseti bunun yerine
koşular arası `beam_map` ile statik olarak tarandı.

**Kısıt**: `beam_gains` değerleri elle konmuş sabitler; bir anten örüntüsünden
ya da UE'nin ışın eksenine göre açısal ofsetinden türetilmiyor. Yani bu, rfsim
ışın mekanizmasının HAPS kanalıyla **toplandığını** ve linki kapatabildiğini
gösteriyor ama fiziksel bir HAPS çoklu-ışın modeli değil — gerçeği için ışın
kazancını `haps_geometry.c`'deki açıya bağlamak gerekir (ayrı bir Adım).

**Test sonucu (Deney 24)**: gNB ışın 0'da sabit, UE ışını taranıyor. Δ=0 (0 dB):
tam performans (2/2 bağlanır, DL BLER ~0). Δ=1 (−4 dB): bağlanır (2/2), DL BLER
biraz artar (~0.002), UE SINR ~1–1.5 dB düşer, UL güç kontrolüyle korunur.
Δ=2 (−9 dB): senkron **başarısız** (0/2) — efektif netgain ~−15, Deney 7'nin
senkron eşiğini geçiyor. Δ=3 (−16 dB): ölü. `HAPS_DEBUG_38811` netgain'i tüm
koşularda ~−6…−7'de kaldı (ışından bağımsız) — ceza gerçekten ayrı bir terim.
