...
| Asimetrik MIMO (2x1/1x2) RRC'ye ulaşmıyordu / cross-matched çözülüyordu | **HAPS'a özel değildi** - `random_channel.c`'nin `load_channellist()`'i uplink/downlink modellerine yön-bazlı olmayan aynı nb_tx/nb_rx veriyordu (bkz. Adım 35); Adım 51'in override'ı eklendikten sonra bile config RU'ları hâlâ çapraz-eşlenmiş bir hack kullanıyordu (Deney 31) | **tam düzeltildi (Adım 51 / Deney 31 + Adım 52 / Deney 33)**: opt-in `n_tx`/`n_rx` per-instance override + her düğüme kendi gerçek anten sayısı, n_pairs=2 her iki yönde uçtan uca kanıtlandı |
...
- MIMO: 1x1/2x2/2x1/1x2 hepsi tam çalışıyor - 2x1/1x2 gerçekten asimetrik (Adım 52 / Deney 33: her düğüme kendi gerçek anten sayısı verildi, n_pairs=2 her iki yönde de uçtan uca kanıtlandı); 4x4 hiç yok
