...
| **Düzeltme (Adım 47 + 48)** | **Adım 47**: `ntn-UlSyncValidityDuration-r17` 240→40 (SIB19 120 s yerine 20 s'de okunuyor → hata ~1.6 km yerine ~80 m). **Adım 48** (`haps_channel.c`): gNB, UE'nin düz-çizgi ekstrapolasyonunun kaçırdığı **eğriliği** hesaplayıp (`R''_gerçek − R''_doğrusal`) işaretli `ta-CommonDrift-r17` alanı üzerinden telafi ettiriyor — her loiter fazında çalışır, hatayı ~4× daha keser. `config_ue.c`'ye dokunulmadı. Sonra: offset 0/15k/25k'da UL tüm koşu boyunca temiz. Partner: (a) ephemeris tazeleme periyodunu kısalt, (b) doğrusal ekstrapolasyonun kaçırdığı 2. derece eğriliği bir drift terimiyle telafi et. |
